...
export default function UploadModal() {
    ...
    const onSubmit: SubmitHandler<FieldValues> = async (values) => {
        try {
            ...
            if (songError) {
                setIsLoading(false)
                return toast.error('Fail to upload a song!')
            }
            ...
            if (imageError) {
                setIsLoading(false)
                return toast.error('Fail to upload an image!')
            }
    ...
    return (
        <Modal title="Add a song" description="Upload a mp3 file"
            isOpen={uploadModal.isOpen} onChange={onChange}>
            <form className="flex flex-col gap-y-4"
                onSubmit={handleSubmit(onSubmit)}>
                <Input id="title" disabled={isLoading} placeholder="Song title"
                    {...register('title', { required: true })} />
                <Input id="author" disabled={isLoading} placeholder="Song author"
